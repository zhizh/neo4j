...
class KernelTransactionsStateBehaviour implements TransactionStateBehaviour {
    ...
    public KernelTransactionsStateBehaviour(StorageEngine storageEngine, ApplyEnrichmentStrategy enrichmentStrategy) {
    ...
    public boolean keepMetaDataForDeletedRelationship() {
        return enrichmentStrategy.check() != EnrichmentMode.OFF
                || storageEngine.transactionStateBehaviour().keepMetaDataForDeletedRelationship();
    }

    @Override
    public boolean useIndexCommands() {
        return storageEngine.transactionStateBehaviour().useIndexCommands();
    }
}
