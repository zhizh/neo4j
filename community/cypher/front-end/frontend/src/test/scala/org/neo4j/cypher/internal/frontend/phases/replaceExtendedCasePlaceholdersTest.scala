...
class replaceExtendedCasePlaceholdersTest extends CypherFunSuite with AstConstructionTestSupport with Inside {
  ...
  def rewriterUnderTest: Rewriter =
    replaceExtendedCasePlaceholders.rewriter(new AnonymousVariableNameGenerator())(CancellationChecker.NeverCancelled)
  ...
  test("case expression with placeholders is rewritten to already existing variables") {
    val input = caseExpression()
      .withCandidate(varFor("a"))
      .withExtendedCase(equals(_, literal("b")), literal("c"))
      .withExtendedCase(equals(_, literal("d")), literal("e"))
    ...
    inside(output) {
      case CaseExpression(candidate, _, alternatives, _) =>
        candidate shouldBe input.candidate
        alternatives shouldBe Vector(
          equals(varFor("a"), literal("b")) -> literal("c"),
          equals(varFor("a"), literal("d")) -> literal("e")
        )
    }
  }

  test("case expression with different expressions placeholders rewritten to the correct variables") {
    val input = caseExpression()
      .withCandidate(varFor("a"))
      .withExtendedCase(greaterThan(_, literal("b")), literal("c"))
      .withExtendedCase(lessThan(_, literal("d")), literal("e"))
    ...
    inside(output) {
      case CaseExpression(candidate, _, alternatives, _) =>
        candidate shouldBe input.candidate
        alternatives shouldBe Vector(
          greaterThan(varFor("a"), literal("b")) -> literal("c"),
          lessThan(varFor("a"), literal("d")) -> literal("e")
        )
    }
  }
