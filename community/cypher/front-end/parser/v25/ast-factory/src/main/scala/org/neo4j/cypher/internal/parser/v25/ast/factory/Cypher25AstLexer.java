...
public final class Cypher25AstLexer extends Cypher25Lexer implements CypherQueryAccess {
    private final String inputQuery; // Note, not always identical to what the parser sees.
    private final int[] offsetTable;

    private Cypher25AstLexer(CharStream input, String inputQuery, int[] offsetTable) {
        super(input);
     ...
     */
    public static Cypher25AstLexer fromString(final String cypher, boolean fullTokens) throws IOException {
        return fromString(cypher, UnicodeEscapeReplacementReader.DEFAULT_BUFFER_SIZE, fullTokens);
    }

    @VisibleForTesting
    public static Cypher25AstLexer fromString(final String cypher, int bufferSize, boolean fullTokens)
            throws IOException {
        final var read = UnicodeEscapeReplacementReader.read(cypher, bufferSize);
        final var lexer = new Cypher25AstLexer(read.charStream(), cypher, read.offsetTable());
